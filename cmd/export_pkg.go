...
var exportPkgCmd = &cobra.Command{
	Use:   "pkg",
	Short: "Export native packages (.deb, .rpm, .pkg.tar.zst, .apk) to remote server",
...
func handleExportPkg(clean bool) {
	...
	switch family {
	case "debian":
		pattern = "penguins-wardrobe*.deb"
		extension = ".deb"
	case "archlinux", "manjaro":
		pattern = "penguins-wardrobe*.pkg.tar.zst"
		extension = ".pkg.tar.zst"
	case "fedora", "opensuse":
		pattern = "penguins-wardrobe*.rpm"
		extension = ".rpm"
	case "alpine":
		pattern = "penguins-wardrobe*.apk"
		extension = ".apk"
	default:
	...
	socketPath := "/tmp/wardrobe-ssh-mux-pkg"
