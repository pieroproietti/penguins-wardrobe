...
var exportLogCmd = &cobra.Command{
	Use:   "log",
	Short: "Export logs in a single command",
	Run: func(cmd *cobra.Command, args []string) {
		...
		files := []struct {
			...
			{"/var/log/wardrobe.log", "wardrobe.log"},
		}

		if reports, err := filepath.Glob("/var/log/wardrobe/wardrobe-report-*.txt"); err == nil && len(reports) > 0 {
			var latestReport string
			var latestTime time.Time
			for _, r := range reports {
				if info, err := os.Stat(r); err == nil && info.ModTime().After(latestTime) {
			...
			if latestReport != "" {
				files = append(files, struct {
					LocalPath  string
					RemoteName string
				}{latestReport, filepath.Base(latestReport)})
			}
		}
